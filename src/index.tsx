...
import './index.css';
import { App } from './App';
import { DispatcherSession, GatewayRequest, Platform, Result } from '@uniscale-sdk/ActorCharacter-RunwellX';
...
import { Patterns as AccountPatterns } from '@uniscale-sdk/ActorCharacter-RunwellX/sdk/RunwellDemo/Account/Patterns';
import { Patterns as StreamsPatterns } from '@uniscale-sdk/ActorCharacter-RunwellX/sdk/RunwellDemo/Streams';
...
const initializeApp = async (): Promise<DispatcherSession> => {
  const session = await Platform.builder()
    .withInterceptors(i => {
      ...
      i.interceptPattern("RunwellDemo.Account_1_0.*", async (input, ctx) => {
      ...
      i.interceptPattern("RunwellDemo.Streams_1_0.*", async (input, ctx) => {
          const headers = { 'Content-Type': 'application/json' };
          // To call the feature we call a POST method on the right URL
          // and pass it a serialized GatewayRequest as define in the SDK
          const response = await axios.post(
  ...
  return session.asSolution('fea57070-a9dc-4018-b74d-e5c07491972b')
}
